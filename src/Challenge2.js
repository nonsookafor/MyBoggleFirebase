...
function Challenge2({setTotalWords, highScore2, setHighScore2, setGridNumber, missedWords, setMissedWords, setTime, chosenWords, setChosenWords, solution, setSolution}) {
	...
	function updateState() {
		...
		const hRef = doc(db, "LoadChallenge2", 
"kJHpWh05rijooDqi4HIH");
		updateDoc(hRef, {
			highScore: highScore2
		})

		if (validWord.length > highScore2) {
			setHighScore2(validWord.length);
			updateDoc(hRef, {
  ...
  function endGame() {
	...
	function check(e) {
		let solution = getGrid(7)[1];
		var word = input;
		console.log(getGrid(7)[1]);
